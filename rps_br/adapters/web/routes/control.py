...
class ElevationMaskRequest(BaseModel):
    elevation_mask_deg: float = Field(..., ge=0.0, le=45.0, description="Ângulo de corte da máscara de elevação em graus")
...
def set_elevation_mask(req: ElevationMaskRequest) -> Dict[str, Any]:
    hub = TelemetryHub.get_instance()
    hub.set_elevation_mask(req.elevation_mask_deg)
    return {"message": "Máscara de elevação atualizada", "elevation_mask_deg": req.elevation_mask_deg}
